...
def get_currency_id(currency_code="NGN"):
    query = "SELECT currency_id FROM currencies WHERE UPPER(currency_code) = %s LIMIT 1"
    result = pd.read_sql(query, engine, params=(currency_code.upper(),))
    if result.empty:
        raise ValueError(f"Currency code {currency_code} not found in currencies table")
...
def run_fx_simulation(currency_code="NGN", days=90, simulations=10000):
    """
    ...
    -----------
    currency_code : str
        Currency code to simulate (default NGN)
    days : int
...
def run_supplier_risk():
    ...
    # Refresh table data without dropping schema/constraints
